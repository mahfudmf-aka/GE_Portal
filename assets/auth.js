...
function gxHasPermission(permission){const s=gxGetSession();if(!s)return false;if(s.role==='Super Admin')return true;if(s.role==='Admin'||String(s.accessLevel||'')==='Admin'){if(permission==='admin')return true;const p=gxUserPermissions();if(p.length)return p.includes(permission);const tabs=gxUserTabs();if(tabs.length)return tabs.includes('ALL')||tabs.includes(permission);return false}const p=gxUserPermissions();if(p.length)return p.includes(permission);if(gxIsExternal()){if(['initiatives','calendar','project-tracking','news','contact','inbox'].includes(permission))return true;return gxUserPermissions().includes(permission)||gxUserTabs().includes(permission)||gxUserTabs().includes('ALL')}return gxHasTab(permission)}
