...
function gxCanonicalRoute(page,search='',hash=''){const raw=String(page||'').toLowerCase();if(raw==='login.html'||raw==='service.html')return `${raw}${search}${hash}`;if(raw==='app.html'){const p=new URLSearchParams(search);const route=(p.get('page')||'index').trim().toLowerCase();return `app.html?page=${encodeURIComponent(route)}${hash}`;}const route=raw.replace(/\.html$/,'');return `app.html?page=${encodeURIComponent(route)}${hash}`}
function gxCurrentRoute(){const page=(location.pathname.split('/').pop()||'app.html');return gxCanonicalRoute(page,location.search||'',location.hash||'')}
function gxSafeReturnTo(value){const raw=String(value||'').trim();if(!raw)return'';try{const url=new URL(raw,location.href);if(url.origin!==location.origin)return'';const page=(url.pathname.split('/').pop()||'app.html').toLowerCase();if(!/^[a-z0-9][a-z0-9._-]*\.html$/i.test(page)||page==='login.html')return'';if(page==='app.html'){const route=(url.searchParams.get('page')||'index').trim().toLowerCase();if(!/^[a-z0-9][a-z0-9._-]*$/i.test(route))return'';return `app.html?page=${encodeURIComponent(route)}${url.hash}`}return gxCanonicalRoute(page,url.search,url.hash)}catch(e){return''}}
...
function gxEnforcePageAccess(){const file=(location.pathname.split('/').pop()||'app.html').toLowerCase();const page=file==='app.html'?`${(new URLSearchParams(location.search).get('page')||'index').trim().toLowerCase()}.html`:file;if(page==='login.html'||page==='service.html')return;const s=gxGetSession();if(page==='index.html'&&s&&window.GXDashboardPOV&&window.GXDashboardPOV(s)==='unresolved')return;const permission=GX_PAGE_PERMISSION_MAP[page];if(permission&&!gxHasPermission(permission)){const target=gxDefaultPage();if(target!==gxCurrentRoute())location.replace(target)}}
