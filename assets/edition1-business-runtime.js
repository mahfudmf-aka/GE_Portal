...
function gantt(rows,tp){rows=[...rows].sort((a,b)=>String(a[geGanttSortStateR13.key]||'').localeCompare(String(b[geGanttSortStateR13.key]||''),undefined,{numeric:true})*geGanttSortStateR13.dir);const {min,max}=geScaleRangeR16(rows),span=Math.max(864e5,max-min),h=geTimelineHeaderR16(min,max),heads=h.cells.map(c=>`<div style="flex:${c.days}"><b>${esc(c.label)}</b><small>${esc(c.sub)}</small></div>`).join(''),arrow=k=>geGanttSortStateR13.key===k?(geGanttSortStateR13.dir>0?' ↑':' ↓'):'',today=new Date();today.setHours(0,0,0,0);const todayPct=(today-min)/span*100,todayLine=todayPct>=0&&todayPct<=100?`<i class="ge-gantt-today-r16" style="left:${todayPct}%" title="Hari ini"></i>`:'';return `<div class="ge-gantt-r13"><div class="ge-gantt-title-r13"><div><b>Gantt Project Tracking</b><small>${min.toLocaleDateString('id-ID')} – ${max.toLocaleDateString('id-ID')}</small></div><div class="ge-gantt-legend-r13"><span class="initiative">Initiative</span><span class="milestone">Milestone</span><span class="activity">Activity</span></div></div><div class="ge-gantt-grid-r13"><div class="ge-gantt-head-r13 ge-gantt-project-head-r13"><button onclick="geGanttSortR13('title')">Project / Milestone / Activity${arrow('title')}</button><i class="ge-gantt-resize-r13" title="Geser untuk mengubah lebar kolom"></i></div><div class="ge-gantt-head-r13 ge-gantt-deadline-head-r13"><button onclick="geGanttSortR13('due')">Deadline${arrow('due')}</button></div><div class="ge-gantt-periods-r13">${heads}</div>${rows.map(r=>{const a=geDate(r.start||r.due),b=geDate(r.due||r.start),left=a?Math.max(0,Math.min(100,(a-min)/span*100)):0,width=b&&a?Math.max(.7,Math.min(100-left,(b-a)/span*100)):.7,deadline=b?Math.max(0,Math.min(100,(b-min)/span*100)):null,tip=`${r.title} • PIC: ${r.pic||'-'} • ${r.start||'-'} – ${r.due||'-'} • ${r.status||'-'}`;return `<button class="ge-gantt-label-r13 ${r.kind.toLowerCase()}" data-gantt-id="${esc(r.id)}" onclick="geV2554OpenGanttItem('${esc(r.id)}')"><i class="${r.kind.toLowerCase()}"></i><span>${esc(r.title)}</span></button><button class="ge-gantt-deadline-r13" onclick="geV2554OpenGanttItem('${esc(r.id)}')">${esc(r.due||'—')}</button><div class="ge-gantt-track-r13">${todayLine}<button title="${esc(r.title)} • PIC: ${esc(r.pic||'-')} • ${esc(r.start||'-')} – ${esc(r.due||'-')} • ${esc(r.status||'-')}" data-gantt-id="${esc(r.id)}" onclick="geV2554OpenGanttItem('${esc(r.id)}')" class="ge-gantt-bar-r13 ${r.kind.toLowerCase()}" style="left:${left}%;width:${width}%"></button>${deadline!==null?`<i class="ge-gantt-deadline-marker-r16 ${r.kind.toLowerCase()}" style="left:${deadline}%" title="Deadline ${esc(r.due)}"></i>`:''}</div>`}).join('')||'<div class="ge-gantt-empty-r13">Belum ada data.</div>'}</div></div>`}
