...
/* Edition 1 save authority remains the Firestore-backed GEStore. */
...
/* Calendar / Project / Gantt are three top-level views of the same projectEvents + Initiative timeline data. */
function projectRows(){const out=[];(S().initiatives||[]).forEach(i=>{out.push({kind:'Initiative',initiative:i.name||'-',title:i.name||'-',pic:i.pic||'-',start:i.startDate||'',due:i.dueDate||i.endDate||'',status:i.status||'',progress:Number(i.real||0)});(i.workflow||[]).forEach(m=>out.push({kind:'Milestone',initiative:i.name||'-',title:m.title||'Milestone',pic:m.pic||i.pic||'-',start:m.startDate||i.startDate||'',due:m.dueDate||m.endDate||'',status:m.status||'',progress:Number(m.real||0)}))});(S().events||[]).filter(e=>e.source==='Manual').forEach(e=>out.push({kind:'Activity',initiative:'-',title:e.title||'-',pic:e.pic||'-',start:e.date||'',due:e.date||'',status:e.status||e.category||'',progress:0}));return out}
function ensureWorkspace(){const ws=document.querySelector('.project-workspace-v253');if(!ws)return null;let tabs=document.getElementById('geWorkspaceTabsR8');if(!tabs){tabs=document.createElement('div');tabs.id='geWorkspaceTabsR8';tabs.className='ge-workspace-tabs-r8';tabs.innerHTML='<button class="active" data-view="calendar">Calendar</button><button data-view="project">Project</button><button data-view="gantt">Gantt</button>';ws.parentNode.insertBefore(tabs,ws);tabs.querySelectorAll('button').forEach(b=>b.onclick=()=>window.geSetCalendarWorkspaceR8(b.dataset.view));}let alt=document.getElementById('geWorkspaceAltR8');if(!alt){alt=document.createElement('section');alt.id='geWorkspaceAltR8';alt.className='ge-workspace-alt-r8';alt.style.display='none';ws.insertAdjacentElement('afterend',alt)}return {ws,tabs,alt}}
window.geSetCalendarWorkspaceR8=function(view){const x=ensureWorkspace();if(!x)return;x.tabs.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.view===view));x.ws.style.display=view==='calendar'?'':'none';x.alt.style.display=view==='calendar'?'none':'block';if(view==='calendar'){window.geCalRenderV2533?.();return}const rows=projectRows();if(view==='project'){x.alt.innerHTML=`<div class="ge-detail-table-r8"><table><thead><tr><th>Type</th><th>Initiative</th><th>Item</th><th>PIC</th><th>Start</th><th>Due</th><th>Status</th><th>Progress</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.kind)}</td><td>${esc(r.initiative)}</td><td><b>${esc(r.title)}</b></td><td>${esc(r.pic)}</td><td>${esc(r.start||'-')}</td><td>${esc(r.due||'-')}</td><td>${esc(r.status||'-')}</td><td>${r.progress}%</td></tr>`).join('')||'<tr><td colspan="8">Belum ada project data.</td></tr>'}</tbody></table></div>`;return}const dates=rows.map(r=>new Date((r.start||r.due||'')+'T00:00:00')).filter(d=>!isNaN(d)),min=dates.length?new Date(Math.min(...dates)):new Date(),max=dates.length?new Date(Math.max(...dates)):new Date(min.getTime()+90*864e5),span=Math.max(1,max-min);x.alt.innerHTML=`<div class="ge-gantt-r8"><div class="ge-gantt-head-r8"><b>Project / Milestone / Activity</b><span>${min.toLocaleDateString('id-ID')} — ${max.toLocaleDateString('id-ID')}</span></div>${rows.map(r=>{const a=new Date((r.start||r.due||'')+'T00:00:00'),b=new Date((r.due||r.start||'')+'T00:00:00'),left=isNaN(a)?0:Math.max(0,(a-min)/span*100),width=isNaN(b)?2:Math.max(2,(b-a)/span*100);return `<div class="ge-gantt-row-r8"><div><small>${esc(r.kind)}</small><b>${esc(r.title)}</b><span>${esc(r.pic)}</span></div><div class="ge-gantt-track-r8"><i style="left:${left}%;width:${Math.min(100-left,width)}%"></i></div></div>`}).join('')||'<div class="planning-empty">Belum ada data Gantt.</div>'}</div>`};
window.geInitCalendarWorkspaceR8=function(){ensureWorkspace();window.geSetCalendarWorkspaceR8('calendar');const pic=document.getElementById('geV251EventPicR2');if(pic&&pic.tagName!=='SELECT')replaceWithUser('geV251EventPicR2','PIC (User & Access)',pic.value)};
