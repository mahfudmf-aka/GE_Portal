...
function geGanttFilterValuesR15(){const get=id=>document.getElementById(id)?.value||'';return{journey:get('geGanttJourneyR15'),touchpoint:get('geGanttTouchpointR15')||new URLSearchParams(location.search).get('touchpoint')||'',station:get('geGanttStationR15'),pic:get('geGanttPicR15'),kind:get('geGanttKindR15'),initiative:get('geGanttInitiativeR15')}}
function filteredRows(){const f=geGanttFilterValuesR15();return projectRows().filter(r=>{const init=(store().initiatives||[]).find(i=>String(i.name||'')===String(r.initiative||''))||{};const journeys=arr(init.journeyScopes||init.journey);const sts=arr(init.stations||init.airport);return(!f.journey||journeys.includes(f.journey))&&(!f.touchpoint||r.touchpoints.some(x=>String(x).toLowerCase().includes(String(f.touchpoint).toLowerCase())))&&(!f.station||sts.includes(f.station))&&(!f.pic||String(r.pic||'')===f.pic)&&(!f.kind||String(r.kind||'')===f.kind)&&(!f.initiative||String(r.initiative||'')===f.initiative)})}
function geGanttFiltersR15(){const rows=projectRows(),uniq=a=>[...new Set(a.filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),'id')),opt=(vals,label)=>`<option value="">${label}</option>`+vals.map(v=>`<option>${esc(v)}</option>`).join(''),q=new URLSearchParams(location.search).get('touchpoint')||'';return `<div class="ge-gantt-filterbar-r15"><select id="geGanttJourneyR15">${opt(journeys,'Semua Journey Scope')}</select><select id="geGanttTouchpointR15">${opt(uniq(rows.flatMap(r=>r.touchpoints)),'Semua Touch Point')}</select><select id="geGanttStationR15">${opt(stations(),'Semua Station')}</select><select id="geGanttPicR15">${opt(uniq(rows.map(r=>r.pic).filter(x=>x&&x!=='-')),'Semua PIC')}</select><select id="geGanttKindR15">${opt(['Initiative','Milestone','Activity'],'Semua Jenis Event')}</select><select id="geGanttInitiativeR15">${opt(uniq(rows.map(r=>r.initiative).filter(x=>x&&x!=='-')),'Semua Initiative')}</select></div>`}
function geBindGanttFiltersR15(){const q=new URLSearchParams(location.search).get('touchpoint')||'',tp=document.getElementById('geGanttTouchpointR15');if(tp&&q)tp.value=q;document.querySelectorAll('.ge-gantt-filterbar-r15 select').forEach(e=>e.onchange=()=>window.geSetCalendarWorkspace('gantt'))}
