...
const path=()=>{const file=(location.pathname.split('/').pop()||'app.html').toLowerCase();if(file==='app.html'){const r=new URLSearchParams(location.search).get('page')||'index';return `${r}.html`;}return file};
...
const isCanonicalPage=()=>{const key=path().replace(/\.html$/,'');return !!(window.P40_CLEAN_PAGES&&window.P40_CLEAN_PAGES[key]);};
...
function shell(){
 if(document.body.dataset.gxCanonicalShell==='1')return;
 document.body.dataset.gxCanonicalShell='1';
 if(path()==='login.html'||!isCanonicalPage())return;
...
window.GXPortalShellMount=shell;
