...
(function(){
  ...
  function ensureModal(id,html){let m=document.getElementById(id);if(!m){m=document.createElement('div');m.id=id;m.className='modal-backdrop p26-generated-modal';m.innerHTML=html;document.body.appendChild(m)}return m}
  ...
  function formMarkup(u){
    ...
    return `<form id="p26UserForm" novalidate><input type="hidden" id="p26UserId" value="${esc(id)}"><div id="p26UserErrors"></div>
      <div class="p26-form-section"><div><span class="p26-section-kicker">PERSONAL INFORMATION</span><p>Identity dan account identifier. Temporary Password hanya digunakan untuk Firebase Authentication.</p></div><div class="p26-form-grid"><label>Employee Name *<input id="p26Name" placeholder="Contoh: Budi Santoso" required value="${esc(u?.name||'')}"></label><label>Employee Number *<input id="p26EmployeeNo" placeholder="Contoh: 123456" required value="${esc(u?.employeeNo||'')}"></label><label>Email *<input id="p26Email" placeholder="Contoh: budi.santoso@garuda-indonesia.com" type="email" required ${isEdit?'readonly':''} value="${esc(u?.email||'')}"></label><label>Username *<input id="p26Username" placeholder="Contoh: budi.santoso" required ${isEdit?'readonly':''} value="${esc(u?.username||'')}"></label>${isEdit?'':'<label>Temporary Password *<input id="p26Password" placeholder="Contoh: TempPass#2026" type="password" minlength="10" autocomplete="new-password" required></label><label>Confirm Temporary Password *<input id="p26PasswordConfirm" placeholder="Ulangi temporary password" type="password" minlength="10" autocomplete="new-password" required></label>'}</div></div>
      <div class="p26-form-section"><div><span class="p26-section-kicker">ORGANIZATION</span><p>Role defines organizational context; it does not by itself grant every action.</p></div><div class="p26-form-grid"><label>Unit / Department *<input id="p26Unit" placeholder="Contoh: Ground Experience" required value="${esc(u?.unit||'')}"></label><label>Organization Type *<select id="p26OrgType"><option value="Internal">Internal</option><option value="Branch Office">Branch Office</option><option value="Partner">Partner</option></select></label><label>Role *<select id="p26Role" required>${u&&!ROLE_VALUES.includes(u?.role)?'<option value="" selected>Select organizational role — current legacy role requires review</option>':''}${ROLE_VALUES.map(r=>`<option value="${r}" ${role===r?'selected':''}>${esc(r==='Head Office'?'HeadOffice / Ground Experience Team':r)}</option>`).join('')}</select></label><label>Access Level *<select id="p26Access" required>${ACCESS_VALUES.map(a=>`<option ${access===a?'selected':''}>${a}</option>`).join('')}</select></label><label>Status *<select id="p26Status"><option ${String(u?.status||'Active')==='Active'?'selected':''}>Active</option><option ${u?.status==='Inactive'?'selected':''}>Inactive</option></select></label></div></div>
  ...
  function openUserForm(id=null){
    ...
    modal.querySelector('#p26FormHost').innerHTML=formMarkup(u);modal.setAttribute('role','presentation');modal.classList.add('show');document.body.classList.add('p26-modal-open');
  ...
  function closeUserForm(){document.getElementById('p26UserModal')?.classList.remove('show');document.body.classList.remove('p26-modal-open')}
