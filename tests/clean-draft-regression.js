...
assert(shell.includes("if(path()==='login.html'||path()!=='app.html')return;"),'Canonical shell must own every app.html route.'); assert(shell.includes("if(path()==='login.html'||path()!=='app.html') return null;"),'Canonical shell guard must not whitelist individual pages.');
...
for(const asset of ['assets/firebase-config.js','assets/firebase-client.js','assets/edition1-store.js']) assert(registry.includes(asset),`Canonical runtime registry missing ${asset}`); assert(app.includes('assets/portal-shell.js?v=10.5'),'Canonical app shell must be loaded once by app.html.'); assert(!registry.includes('assets/portal-shell.js'),'Page registry must not load a second shell.');
