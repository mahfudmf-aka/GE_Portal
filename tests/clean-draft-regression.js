...
assert(JSON.stringify(html)==JSON.stringify(['app.html','login.html']),`Clean architecture must remain 2 root HTML files; found: ${html.join(', ')}`);
...
assert(app.includes('clean-shell-boot'),'Canonical app must start in shell-boot state.');
assert(shell.includes("const path=()=>{const file=(location.pathname.split('/').pop()||'app.html').toLowerCase();if(file==='app.html'"),'Portal shell must resolve app.html?page=... as the active logical page.');
assert(shell.includes("clean-shell-ready"),'Portal shell must expose the shell only after canonical mount.');
assert(registry.includes('id=\\"dashboardRoot\\"'),'Dashboard route must mount dashboardRoot inside the canonical app outlet.');
const auth=read('assets/auth.js');
assert(auth.includes("return'app.html?page=index'"),'Auth default route must be canonical app.html?page=index.');
assert(auth.includes('function gxCanonicalRoute'),'Auth return/redirect routes must canonicalize to app.html?page=...');
